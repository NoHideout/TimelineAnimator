...
namespace TimelineAnimator.ImSequencer
{
    public sealed class ZoomScrollbar
    {
        ...
        public static bool Draw(string id, ref State s, float height = 14.0f)
        {
            ...
            if (s.MinViewSpan <= 0) s.MinViewSpan = contentSpan * 0.001;

            double dynamicMinSpan = 30.0 / (pxPerUnit > 0 ? pxPerUnit : 1);
            ...
            double viewMinLimit = Math.Max(s.ContentMin, s.ContentMax - s.MinViewSpan);
            s.ViewMin = Math.Clamp(s.ViewMin, s.ContentMin, viewMinLimit);

            double viewMaxLimit = Math.Min(s.ContentMax, s.ViewMin + s.MinViewSpan);
            ...
            uint colBar = ImGui.GetColorU32(ImGuiCol.SliderGrabActive);
            uint colFill = ImGui.GetColorU32(ImGuiCol.TitleBgActive);
            uint colGrip = ImGui.GetColorU32(ImGuiCol.ScrollbarGrab);
            ...
            var thumbMin = new System.Numerics.Vector2(x0, cursor.Y + 2);
            var thumbMax = new System.Numerics.Vector2(x1, cursor.Y + size.Y - 2);
            draw.AddRectFilled(thumbMin, thumbMax, colFill, thumbRounding);

            float gripW = MathF.Max(4f, MathF.Min(10f, (x1 - x0) * 0.15f));
            draw.AddRectFilled(new System.Numerics.Vector2(x0, cursor.Y + 2),
                new System.Numerics.Vector2(x0 + gripW, cursor.Y + size.Y - 2),
                colGrip, thumbRounding);
            draw.AddRectFilled(new System.Numerics.Vector2(x1 - gripW, cursor.Y + 2),
                new System.Numerics.Vector2(x1, cursor.Y + size.Y - 2),
                colGrip, thumbRounding);
            ...
            if (hovered && ImGui.IsMouseClicked(ImGuiMouseButton.Left))
            {
                if (inLeftGrip)
                {
                    s.ActiveId = Handle_Left;
                    s.GrabOffset = 0;
                }
                else if (inRightGrip)
                {
                    s.ActiveId = Handle_Right;
                    s.GrabOffset = 0;
                }
                else if (inCenter)
                {
                    s.ActiveId = Handle_Center;
                    s.GrabOffset = mx - x0;
                }
                else
                {
